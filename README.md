...
+ *off*  - может существовать окно между моментом, когда клиент получил сообщение о фиксации транзакции и реальной фиксации транзакции в WAL на диске
Если не включины синхронные реплики в *synchronous_standby_names*, то *on*, *remote_write* и *local* ведут себя одинаково - при фиксации транзакции сервер ждет записи данных на локальный диск.
...
+ *remote_write* - фиксация транзакции полче того, как синхронный сервер подтвердил, что он получил запись о фиксации транзакции и передал ее ОС (хотя на диск данные могут еще не доехать)
