...
Нужно остановить сервер, а потом скопировать данные любым способом из папки кластера PG. Остановка нужна, так как обычные средства копирования не получают мгновенного снимка ФС. Есть вариант сделать снимок каталога средствами файловой системы, если она поддерживает мгновенные снимки. Возможно, btrfs может справиться, я не проверял.
+ Point-in-Ttime-Recovery (PITR)

Самый продвинутый вариант, когда в качестве начальной точки восстановления мы используем несогласованную копию + WAL логи, которые копились, пока мы копировали каталог БД. Потом мы можем их проиграть и привести базу в согласованное состояние.К тому же, можно не останавливаться и продолжить собирать все последующие WAL логи. При восстановлении мы получаем возможность проиграть их до нужного момента времени и восстанавливаться на любой требуемый момент времени. Вся процедура реализуется через pg_basebackup или программные обертки над ним (я у себя использую Barman).
