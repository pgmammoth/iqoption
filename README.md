...
С моей точки зрения, полезнее всего были lateral, fdw, jsonb, upsert, materializes view и репликация.

*2. Что такое autovacuum?*

Автовакуум физически представляет из себя постоянный фоновый процесс autovacuum launcher, который мониторит изменения в таблицах и по определенным условиям в настройках PG просит postmaster запустить рабочие процессы автоочистки autovacuum worker. Есть много ручек, которые позволяют управлять частотой запуска, количеством процессов, временем работы и нагрузкой на систему и они подбираются эмпиричеки. Есть возможность настраивать эти ручки для каждой таблицы отдельно. Каждый рабочий процесс автоочистки занимается удалением ненужных кортежей из таблиц и индексов (MVCC), сбором статистики и обновлением карты видимости таблиц.
...
+ BRIN - никогда с ним не работал. Знаю, что он используется для очень больших таблиц, где данные лежат в достаточно отсортированном виде, (например, гигантские таблицы логов, строки в которых вставлялись последовательно по времени). Размечает данные по блоковым зонам и осуществляет поиск только в подходящей блоковой зоне (похоже по принципу на партицирование). Требует перепроверки найденных результатов. Так как индекс получается компактный, поиск очень быстрый.
...
```
Возможно, при этом условии таблица будет очень большая и мы захотим вообще исключить затрагивание хипа, тогда можно будет сделать индекс покрывающим и активно обновлять таблицу видимости через autovacuum (freeze)
```sql
...
Репликация по архитектуре делится на master-master и master-slave, а по типу на физическую (передаем бинарные данные) и логическую (передаем выражения). В чистом PG есть только физическая master-slave, остальное - сторонние решения или расширения. Физическая master-slave репликация является развитием идеи PITR, только когда мы не дожидаемся краха мастера, а все время восстанавливаемся на реплику. То есть мастер складывает WAL в архив, а реплика все время их читает из архива и проигрывает у себя. В recovery.conf мы указываем, что реплика не должна останавливаться и все время ждет нового файла в архиве. Ну и в случае сбоя мастера мы создаем файл-триггер, описанный у реплики в recovery.conf, по которому она понимает, что хватит восстанавливаться, пора работать. Если указать на реплике режим *hot_standby*, то к реплике можно будет осуществлять запросы на чтение, как она окажется в согласованном состоянии. Проблемой варианта с воспроизведением WAL из архива на реплике является то, что при чтении WAL файлов реплика как минимум отстает на один незаполненный сегмент. Для решения этой проблемы создана потоковая репликация, когда реплика проигрывает все WAL и не найдя новых, подключается к слоту в мастере (специальные параметры в recovery.conf) и непосредственно оттуда берет поток изменений. Потоковая репликация асинхронна по умолчанию, то есть существует окно, когда данные закоммитились на мастере, но еще не появились на реплике. Можно включить режим синхронной репликации, но это очень опасно в плане производительности. Так же можно настроить каскадную репликацию, когда реплика будет передавать WAL логи зависимой от нее реплике, образуя каскад. Каскадные реплики асинхронны.

*16. Расскажите про назначение настроек synchronous_commit и commit_delay. Когда их стоит и не стоит менять?*

Настройка synchronous_commit определяет, будет ли сервер ждать, пока записи из WAL сохранятся на диск, перед тем как сообщить клиенту о фиксации транзакции. Может принимать следующие значения: *on*, *remote_write*, *local* и *off*.
+ *off*  - может существовать окно между моментом, когда клиент получил сообщение о фиксации транзакции и реальной фиксации транзакции в WAL на диске. В случае сбоя мы потеряем транзакции, которые клиент отметил как зафиксированные.

Если не включены синхронные реплики в *synchronous_standby_names*, то *on*, *remote_write* и *local* ведут себя одинаково - при фиксации транзакции сервер ждет записи данных на локальный диск.
Если синхронные реплики включены, то есть нюансы:
+ *on* - (по умолчанию) фиксация транзакции завершается только после того, как синхронный сервер зафиксировал транцакцию у себя на диск.
+ *remote_write* - фиксация транзакции после того, как синхронный сервер подтвердил, что он получил запись о фиксации транзакции и передал ее ОС (хотя на диск данные могут еще не доехать)
+ *local* - мастер ждет фиксацию транзакции только на свой локальный диск, асинхронный коммит по отношению к реплике.

Не стоит включить off если речь идет про важные данные, вроде денег или чего-то подобного. В случае *synchronous_standby_names* нужно выбирать в зависимости от архитектуры репликации.

Параметр *commit_delay* добавляет задержку в  микросекундах перед записью WAL на диск. Это делается в надежде, что за время задержки прилетят еще данные на фиксацию и мы их всех дружно запишем, сэкономив на дорогостоящей операции записи на диск. Это выгодно в высоконагруженных системах, но бесполезно в слабо нагруженных - просто лишняя задержка в фиксации транзакций.
